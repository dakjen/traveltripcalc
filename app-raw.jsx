...
function TripListScreen({ onSelect, onNew, isMobile }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = JSON.parse(localStorage.getItem(STORAGE_INDEX_KEY) || "null");
...
export default function TripPlanner() {
  ...
  const loadTrip = async (id) => {
    try {
      const res = JSON.parse(localStorage.getItem(tripStorageKey(id)) || "null");
      if (res) {
  ...
  const saveTrip = async () => {
    ...
    try {
      ...
      let index = [];
      try {
        const idxRes = JSON.parse(localStorage.getItem(STORAGE_INDEX_KEY) || "null");
        if (idxRes) index = JSON.parse(idxRes.value);
      } catch {}
